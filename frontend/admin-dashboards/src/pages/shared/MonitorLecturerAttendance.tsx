...
interface Summary {
  ...
  monitored: number; taught: number; missed: number; rate: number; last_patrol_date: string
}
interface Visit {
...
export default function MonitorLecturerAttendance() {
  ...
  const totals = summary.reduce((a, s) => ({ p: a.p + s.monitored, t: a.t + s.taught }), { p: 0, t: 0 })
    ...
    <div style={{ color: 'var(--text)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <ExportButtons base="/api/v1/dashboard/lecturer-attendance/patrol/export"
          filename="lecturer-attendance-monitor" query={qs} disabled={summary.length === 0} />
      </div>
          ...
          {list.map(s => (
            <>
              <tr key={s.lecturer_id} style={{ borderBottom: open.has(s.lecturer_id) ? 'none' : '1px solid var(--border,#f1f5f9)' }}>
                ...
                <td style={{ padding: '10px 12px', fontWeight: 700 }}>{s.monitored}</td>
