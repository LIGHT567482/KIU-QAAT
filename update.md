...
_Last updated: 2026-06-14 (session 12). Previous updates 2026-06-02.
...
## Session 12 — SMTP prep + mobile access + sync wiring (2026-06-14)

### QR SMTP test setup
- Added `students_extended` row (`jzany17@gmail.com`, ACTIVE, Year 1/Sem 1) to `db/seeds/003_e2e_test_data.sql`
- QR batch now reports `estimated=1` — seed student is targeted
- To test with real email: fill `.env.smtp` → `source .env.smtp && ./scripts/start_qr_generator.sh` → run E2E script
- Email should arrive at `jzany17@gmail.com` with QR PNG attachment and subject "Your Test University Attendance QR Code — 2024/2025"

### Mobile / real hardware access
- Coordinator PWA restarted with `VITE_API_URL=http://10.200.6.121:8080 pnpm dev --host`
- **Mobile URL:** `http://10.200.6.121:3000` (same LAN required)
- Login: `coordinator@test.local / Coord1234!` — `tenant_id` resolved automatically via tenant-lookup
- Flow: IDLE → select "Introduction to Programming" → PENDING_LECTURER → select "Dr. Jane Smith" → ACTIVE → students check in → End Session

### Sync round-trip wiring
- `SYNC_RECEIVER_URL=http://127.0.0.1:8083` added to api-gateway env and to `scripts/start_all_local.sh`
- Sync chain now complete: PWA outbox → `${VITE_API_URL}/api/v1/sync/*` → api-gateway proxy → sync-receiver :8083 → `attendance_logs` in Postgres
- To manually trigger sync from DevTools: `(await navigator.serviceWorker.ready).sync.register('qaat-sync-outbox')`
- Verify: `SELECT COUNT(*) FROM attendance_logs WHERE session_id = '<id>'`

---

## Session 11 — Lecturer attendance tracking + E2E test + local startup (2026-06-14)

### Lecturer attendance tracking
The `lecturer_attendance_logs` table existed since migration 001 but nothing wrote to it. Now fully wired:

- **`OpenSession`** — inserts a row with `gate_open_time`, `lecturer_id`, `unit_id`, `venue_id`, `session_date` when `lecturer_id` is set in the request
- **`CloseSession`** — new `POST /api/v1/sessions/{session_id}/close` endpoint (COORDINATOR role):
  - Sets `session_status = 'CLOSED'` and `gate_close_time`
  - Updates `lecturer_attendance_logs.gate_close_time` and calculates `contact_hours = ROUND(EPOCH(close - open) / 3600, 2)`
  - Returns `{ session_id, status, closed_at, checkin_count }`
- **Admin reads** — two new admin endpoints:
  - `GET /api/v1/admin/tenants/{id}/lecturer-attendance` — full log with lecturer name, unit name, times
  - `GET /api/v1/admin/tenants/{id}/lecturer-attendance/summary` — per-lecturer totals (sessions, contact_hours, last_session)
- **Admin dashboard** — `AdminLecturerAttendance.tsx` — summary cards per lecturer + detail log table
- **Coordinator PWA** — `SessionPage.tsx onEnd` calls `CloseSession` before `END_SESSION` state machine event
- **Migration 013** — indexes on `lecturer_attendance_logs` + grants for all 011/012 tables

### SMTP test setup
- `.env.smtp.example` — Gmail/SendGrid/Mailhog config template; copy to `.env.smtp` and fill credentials
- `scripts/start_qr_generator.sh` — starts qr-generator with correct `RSA_PUBLIC_KEY_PATH` + `KEY_ENCRYPTION_KEY`; auto-sources `.env.smtp` if present

### E2E test
- `db/seeds/003_e2e_test_data.sql` — fixed-UUID tenant + admin + coordinator + course + unit + lecturer (used by run_e2e_test.sh)
- `tests/e2e/run_e2e_test.sh` — fully working E2E covering all 8 checks; **all pass as of 2026-06-14**:
  1. Health (api-gateway, qr-generator)
  2. Admin login (tenant_id in request; access_token in response)
  3. Tenant verify via admin API
  4. Coordinator login (device binding key returned)
  5. Manifest pull
  6. Open session (with lecturer_id) → `lecturer_attendance_logs` row created
  7. Close session → DB status=CLOSED + gate_close_time filled
  8. QR batch job queued; sync-receiver health

### Service startup
- `scripts/start_all_local.sh` — builds + starts all 4 services with correct env vars:
  - `auth-service :8090` (JWT_ISSUER=qaat-auth, KEY_ENCRYPTION_KEY)
  - `api-gateway :8080` (AUTH_SERVICE_URL=127.0.0.1:8090, QR_GENERATOR_URL=127.0.0.1:3002)
  - `qr-generator :3002` (RSA_PUBLIC_KEY_PATH, KEY_ENCRYPTION_KEY)
  - `sync-receiver :8083` (SYNC_SIGN_KEY auto-generated)
  - Saves PIDs to `/tmp/qaat-pids.txt` for `kill $(cat /tmp/qaat-pids.txt)` cleanup

---

## Session 10 — data completeness fixes (2026-06-14, second pass)

- `ListTenants` now includes `active_academic_year` + `active_semester` → tenant table badge shows actual value after Set Semester
- `GetCourseRoadmap` now includes `tenant_id` → `AdminCourseUnits.tsx` venues dropdown fetches real venues
- `ListCourses` now includes `total_years` → programme duration shows correctly in courses table
- `CreateCourse` now saves `total_years`; `coordinator_id` inserted as SQL NULL when blank
- `CreateCourseUnit` now saves `default_venue_id`
- api-gateway restarted (PID 238366)

---

## Session 10 additions (2026-06-14)

### Course roadmap + semester control

**New endpoints:**
- `PATCH /api/v1/admin/courses/{course_id}` — update course name/dept/school/coordinator/total_years (admin)
- `PATCH /api/v1/admin/courses/{course_id}/units/{unit_id}` — update unit name/year/semester/academic_year/venue (admin)
- `GET  /api/v1/admin/courses/{course_id}/roadmap` — returns `{ roadmap: {year: {semester: [units]}} }` (admin)
- `PATCH /api/v1/admin/tenants/{tenant_id}/academic-period` — set `active_academic_year` + `active_semester` (admin)

**Migration 012 applied (`db/migrations/012_course_roadmap_and_semester.sql`):**
- `courses.coordinator_id` made nullable — courses can exist before a coordinator is assigned
- `courses.total_years SMALLINT NOT NULL DEFAULT 3` — defines how many years the roadmap spans (1–8)
- `course_units.default_venue_id VARCHAR(50)` — optional default venue for a unit
- `tenants.active_academic_year TEXT NOT NULL DEFAULT ''`
- `tenants.active_semester SMALLINT NOT NULL DEFAULT 0` (0 = show all, 1 or 2 = filter)
- `idx_course_units_roadmap` index on `(course_id, year, semester)`

**Manifest fix:**  
The daily manifest was pulling from `sessions` (circular — coordinator sees nothing before opening a session). Now pulls from `course_units` via the coordinator's assigned courses, filtered by `active_academic_year` and `active_semester`. Coordinator sees ONLY their semester's units until admin sets active period.

**Admin frontend:**
- `AdminCourses.tsx` — inline edit panel per course, total_years picker, coordinator optional
- `AdminCourseUnits.tsx` — redesigned as Year > Semester roadmap grid; "+ Add Unit" pre-fills year+semester; edit per unit
- `AdminTenants.tsx` — "Set Semester" button per tenant; inline panel shows active period badge + form to change academic year and semester

## Session 9 additions (2026-06-14)

### New endpoints added
- `GET/POST /api/v1/admin/tenants/{id}/lecturers` — admin lecturer CRUD (RLS-enforced `lecturers` table)
- `GET/POST /api/v1/admin/tenants/{id}/lecturer-assignments` — assign lecturers to course units
- `DELETE /api/v1/admin/lecturer-assignments/{id}` — remove assignment
- `GET /api/v1/coordinator/units/{unit_id}/lecturers` — coordinator fetches lecturers for a unit (dropdown)
- Migration 011 applied: `lecturers` + `lecturer_assignments` tables with RLS and `idx_lecturer_assignments_unit`

### Auth fixes (session 8, previously applied)
- `DISABLE_MFA=true` env bypasses TOTP for VC/DQA in dev
- `KEY_ENCRYPTION_KEY` wired for coordinator device binding
- Student portal UUID→email→student_id fallback in GetEligibility
