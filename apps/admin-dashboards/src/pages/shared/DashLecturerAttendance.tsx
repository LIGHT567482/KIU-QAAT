...
export default function DashLecturerAttendance() {
  ...
  return (
    <div style={{ color: 'var(--text)' }}>
      ...
      {status === 'ok' && (summary ?? []).length === 0 && <p style={{ color: '#94a3b8' }}>No lecturer attendance recorded yet.</p>}
          ...
          {visibleSummary.map(s => (
            <>
              <tr key={s.lecturer_id} style={{ borderBottom: open.has(s.lecturer_id) ? 'none' : '1px solid var(--border,#f1f5f9)' }}>
                <td style={{ padding: '10px 12px' }}><div style={{ fontWeight: 700 }}>{s.lecturer_name}</div>{s.department && <div style={{ fontSize: 11, color: '#94a3b8' }}>{s.department}</div>}</td>
                ...
                <td style={{ padding: '10px 12px', color: '#64748b' }}>{s.last_session_date ? fmtDate(s.last_session_date) : '—'}</td>
              ...
              {open.has(s.lecturer_id) && (
                <tr key={`${s.lecturer_id}-l`}><td colSpan={6} style={{ padding: '0 12px 14px', background: 'var(--surface,#f8fafc)' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
                    <thead><tr>{['Date', 'Unit', 'Open', 'Close', 'Contact Hrs', 'Status'].map(h => <th key={h} style={{ padding: '6px 10px', textAlign: 'left', color: '#94a3b8' }}>{h}</th>)}</tr></thead>
                    <tbody>
                      ...
                      {logsFor(s.lecturer_id).length === 0 && <tr><td colSpan={6} style={{ padding: 12, color: '#94a3b8' }}>No session logs.</td></tr>}
                    </tbody>
                  </table>
                </td></tr>
              )}
